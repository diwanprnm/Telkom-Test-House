jQuery(function($){
	...
	var _pb_calendar = (function(target_, options_){
		...
		this._target.append('<div class="calendar-head-frame">' +
			'<div class="row row-dayname">' +
				...
				'<div class="col col-dayname holiday">SAT</div>' +
	...
	_pb_calendar.prototype.update_view = (function(){
		...
		var today_yyyymmdd = ( moment(new Date()).format("YYYYMMDD")  );

		var month_srt_moment_ = yyyymm_.clone();
			month_srt_moment_.date(0);

		var month_end_moment_ = yyyymm_.clone();
			month_end_moment_.date(30);
		...
		month_srt_moment_.weekday(2).subtract(1,"days");
		month_end_moment_.weekday(8).subtract(1,"days");
		...
		for(var day_index_= 0; day_index_ <= day_count_; ++day_index_){
			...
			var col_class_name_ = "";
			var is_today_selectable = true;
			var is_today_holiday = false;
			var is_day_in_current_month = true;

			if(target_yyyymm_num_ < yyyymm_num_){
				col_class_name_ = "before-month";
				is_day_in_current_month = false;
			}else if(target_yyyymm_num_ > yyyymm_num_){
				col_class_name_ = "after-month";
				is_day_in_current_month = false;
			}

			if( parseInt(target_moment_.format("YYYYMMDD")) < parseInt(today_yyyymmdd)+this._options['min_clickable_from_today'] ){
				col_class_name_ += " before-today";
				is_today_selectable = false;
			}
			...
			if(!is_day_in_current_month){
				'<div>&nbsp;</div>';
			}else if(this._options['day_selectable'] && is_today_selectable && !is_today_holiday ){
