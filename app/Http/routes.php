...
Route::get('cetakSPB', function(Illuminate\Http\Request $request){
	...
	$pdf->Cell(60);
	$pdf->Cell(70,5,'DIVISI DIGITAL BUSINESS - PT TELKOM',0,0,'C');
	$pdf->Ln();
	$pdf->SetFont('helvetica','B',9);
	$pdf->Cell(60);
	$pdf->SetFont('','BU');
	...
	$pdf->SetAligns(array('C','L','L'));
	$pdf->Row(array('','1. ','Pembayaran dapat dilakukan dengan dua cara, yaitu Transfer dan Virtual Account Bank Mandiri. Saudara'));	
		$pdf->SetXY(128.00125,$pdf->GetY()-5);
		$pdf->Ln();
		$pdf->SetX(37.00125);
		$pdf->Cell(0,5,'wajib melakukan konfirmasi cara pembayaran melalui website Telkom Test House.',0,0,'L');
		$pdf->Ln();
		$pdf->SetFont('ZapfDingbats','', 7);
			...
			$pdf->Ln();
			$pdf->SetFont('helvetica','',9);
			$pdf->SetXY(44.00125,$pdf->GetY()-5);
			$pdf->Cell(0,5,'Untuk',0,0,'L');
			$pdf->SetFont('helvetica','B',9);
			$pdf->SetX(53.00125);
			$pdf->Cell(0,5,'Transfer Bank,',0,0,'L');
			$pdf->SetFont('helvetica','',9);
			$pdf->SetX(76.00125);
			$pdf->Cell(0,5,'transfer ke rekening nomor',0,0,'L');
			$pdf->SetFont('helvetica','B',9);
			$pdf->SetX(115.00125);
			$pdf->Cell(0,5,'131-0096022712 an. TELKOM, Bank Mandiri',0,0,'L');
			$pdf->Ln();
			$pdf->SetX(44.00125);
			$pdf->Cell(0,5,'KCP KAMPUS TELKOM BANDUNG.',0,0,'L');
			$pdf->SetFont('helvetica','',9);
			$pdf->SetX(99.00125);
			$pdf->Cell(0,5,'Transfer rekening',0,0,'L');
			...
			$pdf->SetFont('helvetica','',9);
			$pdf->SetX(44.00125);
			$pdf->Cell(0,5,'yang dibayarkan (untuk memudahkan penerbitan faktur pajak).',0,0,'L');
			$pdf->Ln();
		$pdf->SetFont('ZapfDingbats','', 7);
			$pdf->Cell(22);
			$pdf->Cell(0, 5, "l", 0, 0);
			...
			$pdf->SetFont('helvetica','B',9);
			$pdf->SetX(53.00125);
			$pdf->Cell(0,5,'Virtual Account,',0,0,'L');
			$pdf->SetFont('helvetica','',9);
			$pdf->SetX(78.00125);
			$pdf->Cell(0,5,'mohon untuk mengikuti petunjuk yang ada di website Telkom Test House',0,0,'L');
			$pdf->Ln();
			$pdf->SetX(44.00125);
			$pdf->Cell(0,5,'atau e-mail petunjuk pembayaran. Guna mempercepat proses verifikasi pembayaran,',0,0,'L');
			$pdf->SetFont('helvetica','BU',9);
			$pdf->SetX(165.00125);
			$pdf->Cell(0,5,'sangat',0,0,'L');
			$pdf->Ln();
			$pdf->SetX(44.00125);
			$pdf->SetFont('helvetica','BU',9);
			$pdf->Cell(0,5,'disarankan pembayaran melalui virtual account.',0,0,'L');
			$pdf->Ln();
			$pdf->SetFont('helvetica','',9);
...
/*Footer Manual*/
	...
	$pdf->Ln();
	$pdf->Cell(280,5,"DIVISI DIGITAL BUSINESS",0,0,'C');
	$pdf->Ln(20);
...
Route::get('/cetakKuisioner', array('as' => 'cetakKuisioner', function(Illuminate\Http\Request $request) {
	...
	$pdf->SetFont('helvetica','B',11);
		...
		$pdf->setXY(10.00125, $pdf->getY() + 4); 
		$pdf->Cell(10,4,"Infrasutructure Assurance (IAS) Divisi Digital Business (DDS) PT. Telekomuniasi Indonesia, Tbk.",0,0,'L');
