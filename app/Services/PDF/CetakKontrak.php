...
class CetakKontrak
{
    ...
    { 
        ...
        if ($data[0]['is_loc_test']){
            ...
            $pdf->Cell(44, $charHeight,'Kesepakatan yang tertuang dalam');$pdf->SetFont('','I');$pdf->Cell(23, $charHeight,'technical meeting');;$pdf->SetFont('','');$pdf->Cell(44, $charHeight,'lokasi dalam negeri adalah benar. Pelanggan menyatakan bahwa lingkungan milik',0,1);
            ...
            $pdf->Cell(107,$charHeight,'Telkom Test House (TTH) menerbitkan biaya uji (biaya uji perangkat dan uang harian');$pdf->SetFont('','I');$pdf->Cell(15, $charHeight,'test enginer');;$pdf->SetFont('','');$pdf->Cell(0,$charHeight,') yang nominalnya tertera sesuai Surat',0,1);
            $pdf->Cell(4.5,$charHeight,"",0,0);$pdf->MultiCell(0, $charHeight,'Pemberitahuan Biaya (SPB). Di luar biaya uji ini pelanggan menanggung transportasi dan akomodasi test engineer selama melakukan uji lokasi. Apabila pelanggan tidak melakukan pembayaran sampai akhir tahun berjalan, maka web TTH secara otomatis menghapus permohonan uji ini.',0,'L');
            ...
            $pdf->Cell(4.5,$charHeight,"",0,0);$pdf->Cell(88,5,"Pembatalan uji ini diputuskan selesai dengan Laporan Hasil Uji (LHU)");$pdf->SetFont('','B');$pdf->Cell(22.5,$charHeight,"tidak memenuhi/");$pdf->SetFont('','BI');$pdf->Cell(16,$charHeight,"not comply.",0,1);$pdf->SetFont('','');
            ...
            $pdf->Cell(62,$charHeight,'Pelanggan menerima LHU dan/atau Sertifikat Uji ',0,'L');$pdf->SetFont('','I');$pdf->Cell(24,5,'Quality Assurance',0,'L');$pdf->SetFont('','');$pdf->Cell(0,5,'(QA) setelah pengujian perangkat selesai.',0,1);
            ...
            $pdf->Cell(4.5, $charHeight,"",0,0);$pdf->MultiCell(0, $charHeight,'pelanggan maupun perangkat uji yang tidak terkait item pengujian antara informasi saat pendaftaran dengan informasi setelah laporan.',0,'L');
            $pdf->Cell(4.5, $charHeight,"12.",0,0);
            $pdf->MultiCell(0, $charHeight,'Pelanggan dilarang memberikan gratifikasi dalam bentuk apapun di luar biaya uji. Jika ditemukan pelanggaran ',0,'L');
            $pdf->Cell(4.5, $charHeight,"",0,0);$pdf->Cell(85, $charHeight,'dalam hal ini maka permohonan uji diputuskan selesai dengan LHU',0,0);$pdf->SetFont('','B');$pdf->Cell(22.5,$charHeight,"tidak memenuhi/");$pdf->SetFont('','BI');$pdf->Cell(16,$charHeight,"not comply.",0,1);$pdf->SetFont('','');
        }
        ...
        $pdf->Cell(58,$charHeight, 'Test Engineer TTH', 0, 0, 'C');
        $pdf->Cell(59,$charHeight, 'Officer TTH', 0, 1, 'C');
        $pdf->Ln(15);
