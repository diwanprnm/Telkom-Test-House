...
class CetakKontrak
{
    ...
    { 
        ...
        if ($data[0]['is_loc_test']){
            ...
            $pdf->Cell(35.5, $charHeight,'Kesepakatan yang tertuang dalam');$pdf->SetFont('','I');$pdf->Cell(18.5, $charHeight,'technical meeting');;$pdf->SetFont('','');$pdf->Cell(99, $charHeight,'uji lokasi adalah benar. Pelanggan menyatakan bahwa lingkungan milik pelanggan (laboratorium,',0,0);$pdf->SetFont('','I');$pdf->Cell(14, $charHeight,'test enginer,',0,1);
            $pdf->Cell(4.5,$charHeight,"",0,0);$pdf->SetFont('','');$pdf->Cell(72, $charHeight,'sampel uji, dan alat ukur) sudah siap.',0,1);
            $pdf->Cell(4.5,$charHeight,"3.",0,0);
            $pdf->Cell(92.5,$charHeight,'Telkom Test House (TTH) menerbitkan biaya uji (biaya uji perangkat dan tunjangan harian');$pdf->SetFont('','I');$pdf->Cell(12.5, $charHeight,'test enginer');;$pdf->SetFont('','');$pdf->Cell(0,$charHeight,') yang nominalnya tertera sesuai Surat Pemberitahuan Biaya',0,1);
            $pdf->Cell(4.5,$charHeight,"",0,0);$pdf->Cell(81, $charHeight,'(SPB). Di luar biaya uji ini pelanggan menanggung transportasi dan akomodasi');$pdf->SetFont('','I');$pdf->Cell(13, $charHeight,'test enginer');$pdf->SetFont('','');$pdf->Cell(0,$charHeight,"selama melakukan uji lokasi. Apabila pelanggan tidak melakukan pembayaran",0,1);
            $pdf->Cell(4.5,$charHeight,"",0,0);$pdf->MultiCell(0, $charHeight,'sampai akhir tahun berjalan, maka web TTH secara otomatis menghapus permohonan ini.',0,'L');
            ...
            $pdf->Cell(4.5,$charHeight,"",0,0);$pdf->Cell(72,$charHeight,"Pembatalan uji ini diputuskan selesai dengan Laporan Hasil Uji (LHU)");$pdf->SetFont('','B');$pdf->Cell(18.5,$charHeight,"tidak memenuhi/");$pdf->SetFont('','BI');$pdf->Cell(16,$charHeight,"not comply.",0,1);$pdf->SetFont('','');
            ...
            $pdf->Cell(50,$charHeight,'Pelanggan menerima LHU dan/atau Sertifikat Uji',0,'L');$pdf->SetFont('','I');$pdf->Cell(19,$charHeight,'Quality Assurance',0,'L');$pdf->SetFont('','');$pdf->Cell(0,$charHeight,'(QA) setelah pengujian perangkat selesai.',0,1);
            ...
            $pdf->Cell(4.5, $charHeight,"",0,0);$pdf->Cell(4.5, $charHeight,"b. ",0,0);$pdf->Cell(28.5, $charHeight,'Jika keputusan hasil uji QA',0,0); ;$pdf->SetFont('','B');$pdf->Cell(19,$charHeight,"tidak memenuhi/");$pdf->SetFont('','BI');$pdf->Cell(13,$charHeight,"not comply",0,0);$pdf->SetFont('','');$pdf->Cell(16,$charHeight,"maka pelanggan dapat mengajukan kembali permohonan uji QA ini terhitung 2 (dua) bulan untuk produk",0,1);
            $pdf->Cell(4.5, $charHeight,"",0,0);$pdf->Cell(4.5, $charHeight,"",0,0);$pdf->Cell(111, $charHeight,'Tingkat Komponen Dalam Negeri (TKDN) dan 6 (enam) bulan untuk produk non TKDN setelah keputusan tersebut keluar.',0,1);
            $pdf->Cell(4.5, $charHeight,"13.",0,0);
            $pdf->Cell(32.5, $charHeight,"Pelanggan dapat menghubungi");$pdf->SetFont('','I');$pdf->Cell(18,$charHeight,"customer service");$pdf->SetFont('','');$pdf->Cell(38.5,$charHeight,"TTH (WA: 081224837500 atau email:");$pdf->SetFont('','U');$pdf->SetTextColor(0,0,255);$pdf->Cell(19,$charHeight,"cstth@telkom.co.id");$pdf->SetTextColor(0,0,0);$pdf->SetFont('','');$pdf->Cell(10,$charHeight,") jika ada perbedaan informasi pelanggan maupun perangkat",0,1);
            $pdf->Cell(4.5, $charHeight,"",0,0);$pdf->MultiCell(0, $charHeight,'uji yang tidak terkait item pengujian antara informasi saat pendaftaran dengan informasi setelah laporan.',0,'L');
            $pdf->Cell(4.5, $charHeight,"14.",0,0);
            $pdf->MultiCell(0, $charHeight,'Pelanggan dilarang memberikan gratifikasi dalam bentuk apapun di luar biaya uji, transportasi, dan akomodasi. Jika ditemukan pelanggaran dalam hal ini maka',0,'L');
            $pdf->Cell(4.5, $charHeight,"",0,0);$pdf->Cell(49.5, $charHeight,'permohonan uji diputuskan selesai dengan LHU',0,0);$pdf->SetFont('','B');$pdf->Cell(18.5,$charHeight,"tidak memenuhi/");$pdf->SetFont('','BI');$pdf->Cell(16,$charHeight,"not comply.",0,1);$pdf->SetFont('','');
        }
        else{
            $pdf->Cell(4.5,$charHeight,"2.",0,0);
            ...
            $pdf->Cell(4.5,$charHeight,"",0,0);$pdf->Cell(4.5, $charHeight,"b. ",0,0);$pdf->MultiCell(0, $charHeight,'Terdapat perubahan referensi uji pada periode nomor SPK telah terbit.',0,'L');
            $pdf->Cell(4.5,$charHeight,"",0,0);$pdf->Cell(4.5, $charHeight,"c. ",0,0);$pdf->MultiCell(0, $charHeight,'Alat ukur mendadak rusak dan tidak dapat diperbaiki lagi.',0,'L');
            $pdf->Cell(4.5,$charHeight,"",0,0);$pdf->Cell(69,$charHeight,"Pembatalan uji diputuskan selesai dengan Laporan Hasil Uji (LHU)");$pdf->SetFont('','B');$pdf->Cell(18.5,$charHeight,"tidak memenuhi/");$pdf->SetFont('','BI');$pdf->Cell(13,$charHeight,"not comply ",0,0);$pdf->SetFont('','');$pdf->Cell(22.5,$charHeight,"sedangkan pembatalan kalibrasi diputuskan selesai dengan",0,1);
            ...
            $pdf->Cell(4.5,$charHeight,"8.",0,0);
            $pdf->Cell(0, $charHeight,'Dalam hal ada pengecekan/perubahan/peminjaman/penggantian/pengambilan perangkat uji oleh pelanggan setelah terbit nomor SPK, maka permohonan ini akan',0,1);
            $pdf->Cell(4.5,$charHeight,"",0,0);$pdf->Cell(33, $charHeight,'diputuskan selesai dengan LHU',0,0);$pdf->SetFont('','B');$pdf->Cell(18.5,$charHeight,"tidak memenuhi/");$pdf->SetFont('','BI');$pdf->Cell(16,$charHeight,"not comply.",0,1);$pdf->SetFont('','');
            $pdf->Cell(4.5, $charHeight,"9.",0,0);
            $pdf->Cell(50,$charHeight,'Pelanggan menerima LHU dan/atau Sertifikat Uji',0,'L');$pdf->SetFont('','I');$pdf->Cell(19,$charHeight,'Quality Assurance',0,'L');$pdf->SetFont('','');$pdf->Cell(70,$charHeight,'(QA) setelah pengujian perangkat selesai atau pelanggan menerima',0,0);$pdf->SetFont('','I');$pdf->Cell(28, $charHeight,'Calibration Certificate',0,1);
            $pdf->Cell(4.5, $charHeight,'',0,0);$pdf->SetFont('','');$pdf->Cell(22.5,$charHeight,"setelah kalibrasi alat ukur selesai.",0,1);
            $pdf->Cell(4.5, $charHeight,"10.",0,0);
            $pdf->Cell(69, $charHeight,'Sebagai prasyarat mengunduh LHU dan/atau Sertifikat Uji QA atau',0,0);$pdf->SetFont('','I');$pdf->Cell(22, $charHeight,'Calibration Certificate',0,0);$pdf->SetFont('','');$pdf->Cell(62, $charHeight,', pelanggan harus mengambil perangkat uji/alat ukur dan mengisi survei',0,1);
            $pdf->Cell(4.5, $charHeight,'',0,0);$pdf->Cell(22.5,$charHeight,"kepuasan pelanggan yang ada di web TTH pada saat pelanggan mengunduh dokumen tersebut.",0,1);
            $pdf->Cell(4.5, $charHeight,"11.",0,0);
            $pdf->MultiCell(0, $charHeight,'Pelanggan bersedia jika informasi pelanggan dan perangkat uji yang telah lulus uji QA dipublikasikan melalui web TTH.',0,'L');
            $pdf->Cell(4.5, $charHeight,"12.",0,0);
            $pdf->MultiCell(0, $charHeight,'Khusus untuk permohonan uji QA, pelanggan harus memperhatikan hal-hal berikut:',0,'L');
            $pdf->Cell(4.5, $charHeight,"",0,0);$pdf->Cell(4.5, $charHeight,"a. ",0,0);$pdf->MultiCell(0, $charHeight,'Pelanggan harus memastikan bahwa sertifikat ISO maupun CIQS valid sebagai prasyarat utama untuk mengeluarkan keputusan hasil uji QA.',0,'L');
            $pdf->Cell(4.5, $charHeight,"",0,0);$pdf->Cell(4.5, $charHeight,"b. ",0,0);$pdf->Cell(28, $charHeight,'Jika keputusan hasil uji QA',0,0);$pdf->SetFont('','B');$pdf->Cell(19,$charHeight,"tidak memenuhi/");$pdf->SetFont('','BI');$pdf->Cell(13,$charHeight,"not comply",0,0);$pdf->SetFont('','');$pdf->Cell(16,$charHeight,"maka pelanggan dapat mengajukan kembali permohonan uji QA ini terhitung 2 (dua) bulan untuk produk",0,1);
            $pdf->Cell(4.5, $charHeight,"",0,0);$pdf->Cell(4.5, $charHeight,"",0,0);$pdf->Cell(111, $charHeight,'Tingkat Komponen Dalam Negeri (TKDN) dan 6 (enam) bulan untuk produk non TKDN setelah keputusan tersebut keluar.',0,1);
            $pdf->Cell(4.5, $charHeight,"13.",0,0);
            $pdf->Cell(33, $charHeight,"Pelanggan dapat menghubungi");$pdf->SetFont('','I');$pdf->Cell(18,$charHeight,"customer service");$pdf->SetFont('','');$pdf->Cell(38.5,$charHeight,"TTH (WA: 081224837500 atau email:");$pdf->SetFont('','U');$pdf->SetTextColor(0,0,255);$pdf->Cell(19.5,$charHeight,"cstth@telkom.co.id");$pdf->SetTextColor(0,0,0);$pdf->SetFont('','');$pdf->Cell(10,$charHeight,") jika ada perbedaan informasi pelanggan maupun perangkat",0,1);
            $pdf->Cell(4.5, $charHeight,"",0,0);$pdf->MultiCell(0, $charHeight,'uji/alat ukur yang tidak terkait item uji antara informasi saat pendaftaran dengan informasi setelah laporan.',0,'L');
            $pdf->Cell(4.5, $charHeight,"14.",0,0);
            $pdf->Cell(111, $charHeight,'Pelanggan dilarang memberikan gratifikasi dalam bentuk apapun di luar biaya uji/kalibrasi. Jika ditemukan pelanggaran dalam hal ini maka permohonan ini diputuskan',0,1);
            $pdf->Cell(4.5, $charHeight,"",0,0);$pdf->Cell(22, $charHeight,'selesai dengan LHU',0,0);$pdf->SetFont('','B');$pdf->Cell(18.5,$charHeight,"tidak memenuhi/");$pdf->SetFont('','BI');$pdf->Cell(13,$charHeight,"not comply ",0,0);$pdf->SetFont('','');$pdf->Cell(5, $charHeight,'atau',0,0);$pdf->SetFont('','BI');$pdf->Cell(16,$charHeight,"Uncalibrated Report. ",0,1);$pdf->SetFont('','');
        }
